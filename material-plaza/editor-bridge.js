...
(function () {
    ...
    // 通过编辑器父窗口转发 GitHub API 请求
    // 直接 fetch 在 iframe 内会因 CORS 策略失败，
    // 从父窗口（编辑器主页面）发起请求可以正常通过
    function forwardGithubFetch(path, options) {
    ...
    window.addEventListener('message', function (e) {
        ...
        // 处理 GitHub API 转发响应
        if (data.type === 'forwardGithubFetchResult') {
    ...
    window.MaterialPlazaBridge = {
        ...
        forwardGithubFetch: forwardGithubFetch,
